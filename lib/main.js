...
if (process.env['CRONICLE_sqlite']) { // use sqlite
...
else if (process.env['CRONICLE_sql_connection_string']) { // use connection string variable
	...
	process.env["CRONICLE_Storage__engine"] = "SQL"
	process.env["CRONICLE_Storage__SQL__client"] = cs['protocol'].slice(0, -1)
	...
	process.env["CRONICLE_Storage__SQL__connection__password"] = decodeURI(cs['password'])
