...
module.exports = Class.create({
	...
	deleteOidcSessionsForClaim: function(issuer, claimType, claimValue, clientId, callback) {
		const self = this;
		const key = OIDC.indexKey(issuer, claimType, claimValue);
		self.storage.get(key, function(err, record) {
			if (err && self.isMissingStorageError(err)) return callback(null, 0);
			if (err) return callback(err);
			if (!record || !record.sessions || typeof record.sessions !== 'object') {
				return callback(new Error('Invalid OIDC session index record'));
			}
			...
			const next = function(deleteErr) {
				...
				if (pos >= ids.length) return callback(null, deleted);
				const id = ids[pos++];
	...
	api_oidc_backchannel_logout: async function(args, callback) {
		...
		self.processLogoutJti(verified.provider.issuer, payload.jti, payload.exp, function(done) {
			self.deleteOidcSessionsForClaim(verified.provider.issuer, claimType, claimValue,
				verified.provider.clientId, done);
