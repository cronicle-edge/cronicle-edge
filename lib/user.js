...
module.exports = Class.create({
	...
	do_ad_auth: async function (user, password) { // return true on success or false in case of error
		...
		const self = this;
		let url = self.server.config.get('ldap_url')
		let bindDN = 'uid=' + user + ',' + self.server.config.get('ldap_bind_dn');
		let ldap_opts = self.server.config.get('ldap_opts') || ""

		self.logDebug(3, "I will login the user using LDAP.");

		const client = new Client({url, ldap_opts});

		try {
			await client.bind(bindDN, password);
			return true;
		} catch (ex) {
			self.logDebug(9, "Couldn't login the user." + ex);
			return false;
		} finally {
			await client.unbind();
		}
