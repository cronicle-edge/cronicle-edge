...
module.exports = Class.create({
	...
	startup: function (callback) {
		...
		// OIDC sessions have related persistent index records which must be
		// removed when the storage maintenance job expires the session.
		...
		// pixl-server-api may log parsed parameters in debug mode.  Move secrets
		// to non-enumerable request properties before the API logger sees them.
		...
		this.server.on('day', ()=>{
			self.oauth_state = {}
			self.oidc_logout_ticket_replay.clear();
	...
	filterOidcRequest: function(args, callback) {
		...
		if (action === 'oidc_logout_redirect') {
			...
			let payload;
			try {
				payload = OIDC.consumeLogoutTicket(secrets.ticket, this.getLogoutTicketSecret());
				if (this.oidc_logout_ticket_replay.has(payload.jti)) throw new Error('Logout ticket was already used');
				OIDC.parseSecureUrl(payload.location, 'OIDC logout redirect', true);
	...
	getLogoutTicketSecret: function() {
			...
			if (err) {
				...
				const rollback = function(currentError) {
					...
					self.mutateOidcIndexKey(
						OIDC.indexKey(session.oidc_issuer, entry.type, entry.value),
	...
	storeOidcSession: function(session, callback) {
		...
		const revocationKeys = self.getOidcIndexEntries(session).map(function(entry) {
			return OIDC.revocationKey(session.oidc_issuer, session.oidc_client_id, entry.type, entry.value);
		});
	...
	processLogoutJti: function(issuer, jti, expires, worker, callback) {
		const self = this;
		const key = OIDC.replayKey(issuer, jti);
	...
	revokeOidcSessionsForClaim: function(issuer, claimType, claimValue, clientId, expires, callback) {
		const self = this;
		const key = OIDC.indexKey(issuer, claimType, claimValue);
		const revocationKey = OIDC.revocationKey(issuer, clientId, claimType, claimValue);
		self.withOidcRevocationLocks([revocationKey], function(done) {
			self.storage.put(revocationKey, { created: Tools.timeNow(true), expires: expires }, function(markerErr) {
				if (markerErr) return done(markerErr);
				self.storage.expire(revocationKey, expires);
				self.storage.get(key, function(err, record) {
	...
	api_callback: async function (args, callback) {
		...
		try {
			...
			if (tokenResponse.id_token && oidcConfigured) {
				const verified = await OIDC.verifyIdToken(tokenResponse.id_token, oauth, stateData.nonce);
			...
			if (!oidcClaims && oauth.issuer && oauth.jwks_url && userResponse.sub) {
				const provider = OIDC.validateProviderConfig(oauth);
				oidcClaims = { iss: provider.issuer, sub: userResponse.sub };
			}
			if (oauth.issuer && oauth.jwks_url) {
				oidcMetadata = OIDC.buildSessionMetadata(oauth, oidcClaims, userResponse, tokenResponse.id_token,
					self.getLogoutTicketSecret());
			}
			...
			self.fireHook('before_logout', args, function (err) {
				...
				try {
					if (session.auth_provider === 'oidc') {
						...
						logoutLocation = OIDC.buildLogoutLocation(oauth, session, self.getLogoutTicketSecret());
					}
				}
				catch (configErr) {
				...
				self.deleteSessionRecord(session, function (deleteErr) {
					...
					if (logoutLocation) {
						try {
							const ticket = OIDC.createLogoutTicket(logoutLocation, self.getLogoutTicketSecret(), 60);
	...
	api_oidc_backchannel_logout: async function(args, callback) {
		...
		const badRequest = function() {
			callback('400 Bad Request', { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' },
				JSON.stringify({ code: 1, description: 'Invalid logout request.' }));
		};
		const serverError = function(status, description) {
			callback(status, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' },
				JSON.stringify({ code: 1, description: description }));
		};

		if (args.request.method !== 'POST') {
			return callback('405 Method Not Allowed', { 'Content-Type': 'application/json', 'Allow': 'POST', 'Cache-Control': 'no-store' },
				JSON.stringify({ code: 1, description: 'Method not allowed.' }));
		}
		...
		try { OIDC.validateProviderConfig(oauth); }
		catch (configErr) {
			self.logError('oauth', 'OIDC back-channel logout is misconfigured: ' + configErr.message);
			return serverError('500 Internal Server Error', 'OIDC logout is not configured correctly.');
		}

		const token = args._oidc_secrets && args._oidc_secrets.logout_token;
		let verified;
		try {
			verified = await OIDC.verifyLogoutToken(token, oauth);
		}
		catch (verifyErr) {
			const code = String(verifyErr.code || '');
			const validationError = code.match(/^ERR_(?:JWT|JWS|JOSE|JWKS)/) ||
				String(verifyErr.message || '').match(/(?:token|claim|signature|nonce|sid|subject|iat|events|size)/i);
			self.logError('oauth', 'OIDC back-channel logout validation failed: ' + (code || verifyErr.message));
			if (validationError) return badRequest();
			return serverError('503 Service Unavailable', 'OIDC signing keys are temporarily unavailable.');
		}
		...
		self.processLogoutJti(verified.provider.issuer, payload.jti, payload.exp, function(done) {
			self.revokeOidcSessionsForClaim(verified.provider.issuer, claimType, claimValue,
				verified.provider.clientId, payload.exp, done);
		}, function(processErr, replay) {
			if (processErr) {
				self.logError('oauth', 'OIDC back-channel logout storage failure: ' + processErr.message);
				return serverError('500 Internal Server Error', 'OIDC logout could not be completed.');
			}
