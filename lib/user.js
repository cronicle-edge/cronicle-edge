...
module.exports = Class.create({
	...
	do_ad_auth: async function (user, password) { // return true on success or false in case of error
		if(!user || !password) { // sanity check
			self.logDebug(3, "LDAP login failed", "Missing username or password")
			return false; 
		}

		const self = this;
		let ad_domain = self.server.config.get('ad_domain');  // e.g. corp.company.com 
		let ad_logon_server = self.server.config.get('ad_logon_server') || ad_domain  // in some cases it's still corp.company.com, but can alter if needed
		let opts = self.server.config.get('ldap') || { url: ('ldap://' + ad_logon_server) }

		let force_ldap = self.server.config.get('force_ldap'); // if you need to use LDAP instead of simple ActiveDirectory

		if(force_ldap == "false") {

			self.logDebug(3, "I will login the user using AD.");

			if(!ad_domain) {
				self.logDebug(3, "AD login failed", "ad_domain is missing")
				return false; 
			}

			// form user@domain username
			if( ad_domain ) user = user + '@' + ad_domain
			...
			return new Promise((resolve, reject) => {
				...
				ad.authenticate(user, password, (err, auth) => {
					if (err || !auth) {
						self.logDebug(3, "AD login failed", err)
						resolve(false)
					}
					else { resolve(true) }
				});
		...
		self.logDebug(3, "I will login the user using LDAP.");

		opts["username"] = user;
		opts["userPassword"] = password;

		return async function doLDAPLogin() {
			try {
				let user_from_ad = await authenticate(opts);
				return user_from_ad ? true : false;
			} catch (error) {
				return false;
			}
		}
	...
	api_login: async function (args, callback) {
		...
		if (user.ext_auth) { // do AD auth
			isValidPassword = await self.do_ad_auth(params.username, params.password);
		}
