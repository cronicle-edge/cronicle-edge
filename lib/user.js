...
module.exports = Class.create({
	...
	do_ad_auth: function (user, password, domain) {
		...
		return new Promise((resolve, reject) => {
			let opts = self.config.get('ldap') || { url: ('ldap://' + domain) }
	...
	api_login: async function (args, callback) {
		...
		if (user.ext_auth) { // do AD auth
			...
			// ad_domain is required for simple AD/LDAP auth. It's either DOMAIN\username or smth like corp.mycompany.com
			var ad_domain = self.server.config.get('ad_domain') || 'corp.cronical.com';
			
			// logon server can also be corp.mycompany.com (same as domain), if it has different FDQN specify it in config
			var ad_logon_server = self.server.config.get('ad_logon_server') || ad_domain
			
			// samAccountName@domain can be used instead of DN in AD to bind ("authenticate") user
