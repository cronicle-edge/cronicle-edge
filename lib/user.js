...
module.exports = Class.create({
	...
	api_oauth: function (args, callback) {
		...
		// sanity check, if session_id cookie exists, user likely just logged in
		// but session_id is not set in localStorage by some reason (very unlikely)
		// in this case just redirect user back to original page right away
		if(args.cookies['session_id']) {
			self.logDebug(3, 'session_id cookie already exist, redirecting user back', args.cookies['session_id'])	
