...
module.exports = Class.create({
		...
		self.fireHook('before_login', args, function (err) {
			...
			self.storage.put('sessions/' + session_id, session, function (err, data) {
				if (err) {
					...
					let res = args.response
					res.setHeader('Set-Cookie', `session_id=${session_id}; HttpOnly; Path=/; Max-Age=30; Secure=True; SameSite=Strict`)
	...
	api_resume_session: function (args, callback) {
		...
		if(!session_id) {
			return callback({code: 0, description: 'request is missing session id'})
		}
	...
	sendEmail: function (name, args, callback) {
		...
		if (emails[name]) {
			...
			let  mailOpts = Tools.mergeHashes(this.server.config.get('mail_options'), {
