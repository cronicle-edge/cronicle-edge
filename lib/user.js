...
module.exports = Class.create({
	...
	api_casauth: async function (args, callback) {
		...
		const casUrl = self.server.config.get('cas_url');
		...
		if(session_id) {
			...
			res.writeHead(302, { Location: self.server.config('#' + orig_location) })
			res.end()			
		}
		else if(params.ticket) { // if ticket is provided, validate with CAS server

			let validateurl = casUrl + '/serviceValidate?service=' + encodeURIComponent(self.server.config.get('base_app_url') 
				+ '/api/user/casauth') + '&ticket=' + encodeURIComponent(params.ticket)
				;

			let request = new Request();
			let opts = { method: 'GET'};			
			request.request(validateurl, opts, async function (err, resp, data) {
				if (err) {
					return self.doError('login', "Error occured during CAS authentication. Check the CAS configuration", callback);
				}
				else {
					...
					try {
						...
						casjson = await parser.parseStringPromise(data);
					}
					catch (e) {
						self.logError(3, 'CAS Response parse error', String(data));
						return self.doError('login', "Invalid CAS response", callback);
					}

					const serviceResp = casjson["cas:serviceResponse"];

					if (serviceResp && serviceResp["cas:authenticationSuccess"] && serviceResp["cas:authenticationSuccess"][0]) {
						// CAS ticket is valid, extract user info
						let users = serviceResp["cas:authenticationSuccess"][0]["cas:user"];
						
						if (!users || !users[0]) {
							self.logError(3, 'CAS validation failed, no user in response', String(data));
							return self.doError('login', "Invalid CAS response: No user", callback);
						}
						else {
							let username = users[0];
							// create session for user
							let user = await self.getUserAsync('users/' + self.normalizeUsername(username));
							if (!user) {
								self.logError(3, 'CAS validation succeeded, but user does not exist:', username);
								return self.doError('login', "CAS validation succeeded, but user does not exist: " + username, callback);
							}
							else if (!user.active) {
								self.logError(3, 'CAS validation succeded, but user is disabled:', username);
								return self.doError('login', "CAS validation succeeded, but user disabled: " + username, callback);
							}
							else {
								self.processLogin(user, args, callback);
							}
						}
			...
			self.logDebug(3, 'rRedirecting to CAS login page:', casUrl)	
			//#TODO get api url from config
			res.writeHead(302, { Location: casUrl + "/login?service=" + self.server.config.get('base_app_url') + "/api/user/casauth" });
			res.end()	
		}
	},

	processLogin: async function (user, args, callback) {

		let self = this;

		let avatar_url;
		let orig_location = "Home";
		...
		self.fireHook('before_login', args, function (err) {
			...
			var session_id = Tools.generateUniqueID(64, user.username);
			var session = {
				id: session_id,
			...
			self.logDebug(6, "Logging user in: " + user.username + ": New Session ID: " + session_id, session);
			...
			self.storage.put('sessions/' + session_id, session, function (err, data) {
				if (err) {
					...
					self.logTransaction('user_login', user.username, self.getClientInfo(args));
