...
function getJson(url, options) {
	return new Promise((resolve, reject) => {
		request.request(url, options, function (err, resp, data) {
			if (err) return reject(err);
			resolve({ resp: resp, data: JSON.parse(data.toString()) });
		});
	});
}
...
module.exports = Class.create({
	...
	do_github_oauth: async function (args, code, callback) {
		const self = this;
		const githubHost = self.server.config.get('github_host');
		const clientId = self.server.config.get('github_client_id');
		const clientSecret = self.server.config.get('github_client_secret');
		const githubOrg = self.server.config.get('github_org');
		const insecure = self.server.config.get('github_insecure_skip_verify') === true;

		const tokenResponse = await getJson( `https://${githubHost}/login/oauth/access_token?client_id=${clientId}&client_secret=${clientSecret}&code=${code}`, {
			'method': 'POST',
			headers: { 'accept': 'application/json' },
			'rejectUnauthorized': !insecure
		});

		const userResponse = await getJson( `https://api.${githubHost}/user`, {
			'method': 'GET',
			headers: { 'accept': 'application/json', 'Authorization': 'token ' + tokenResponse.data.access_token },
			'rejectUnauthorized': !insecure
		});

		const orgResponse = await getJson( userResponse.data.organizations_url, {
			'method': 'GET',
			headers: { 'accept': 'application/json', 'Authorization': 'token ' + tokenResponse.data.access_token },
			'rejectUnauthorized': !insecure
		});

		// validate Org membership if required
		if (githubOrg.length > 0 && !orgResponse.data.some(o => o.login === githubOrg)) {
			return self.doError('github_oauth get orgs', "User authenticated but is not part of correct Github Organization! - Please contact admin", callback);
		} else if (githubOrg.length > 0 && orgResponse.data.some(o => o.login === githubOrg)) {
			self.logDebug(7, "User is part of correct Github org!");
		}

		// similiar to external_login from here on -> refactor
		...
		return new Promise((resolve, reject) => {
			self.storage.get(path, function (err, user) {
				...
				// sync user info
				user.full_name = userResponse.data.name || username;
				user.email = userResponse.data.email || (username + '@' + self.server.hostname);
				user.avatar = userResponse.data.avatar_url || '';

				// save user locally
				self.storage.put(path, user, function (err) {
					if (err) return self.doError('github_oauth', "Failed to create user: " + err, callback);
	...
	api_login: async function (args, callback) {
		...
		var user = null;
		if (code) {
			user = await self.do_github_oauth(args, code, callback);
			params.username = user.username;
		}

		// if we already have the github user, we do not want to fetch/validate anything here
		if (!user) {			
			...
			user = await getUserAsync('users/' + this.normalizeUsername(params.username));
		}
		...
		if (user.ext_auth && user.auth_provider === 'ad') {
		...
		} else if (!user.auth_provider) {  // do local auth
			isValidPassword = self.comparePasswords(params.password, user.password, user.salt)
		}

		if (!isValidPassword && user.auth_provider !== 'github') {
			// incorrect password
	...
	api_external_login: function (args, callback) {
		...
		var request = new PixlRequest(args.request.headers['user-agent'] || 'PixlUser API');
		request.get(url, {
