...
module.exports = Class.create({
		...
		return new Promise((resolve, reject) => {
			...
			ad.authenticate(user, password, (err, auth) => {
				if (err || !auth) { resolve(false) 
				  self.logDebug(3, 'ad login error', err || auth)
				}
				else { resolve(true) }
			});
		});
	...
	api_login: async function (args, callback) {
		...
		var self = this;
		self.logDebug(3, "login api", new Date())
		...
		process.nextTick(()=>{
			self.logDebug(3, "login api -next tick ", user)
		});

		if(!user) {
			self.logDebug(3, "failed to fetch user, try again")
			let user = await getUserAsync('users/' + this.normalizeUsername(params.username));
		}

		if (!user) {
			return self.doError('login', "Username or password incorrect1.", callback); // deliberately vague
		}
		...
		if (!isValidPassword) {
			...
			self.storage.put('users/' + self.normalizeUsername(params.username), user, function (err, data) {
				...
				return self.doError('login', "Username or password incorrect2.", callback); // deliberately vague
			});
		...
		self.fireHook('before_login', args, function (err) {
			...
			self.storage.put('sessions/' + session_id, session, function (err, data) {
				if (err) {
					...
					self.logDebug(6, "Successfully logged in1");
			...
			self.fireHook('before_resume_session', args, function (err) {
				...
				self.storage.put('sessions/' + session.id, session, function (err, data) {
					if (err) {
						...
						self.logDebug(6, "Successfully logged in2");
						...
						self.storage.put(path, user, function (err) {
							...
							self.fireHook('before_login', args, function (err) {
								...
								self.storage.put('sessions/' + session_id, session, function (err, data) {
									...
									self.logDebug(6, "Successfully logged in3", username);
