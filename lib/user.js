...
module.exports = Class.create({
	...
	startup: function (callback) {
		...
		// -----------------  OAUTH ------------------------------------------------
		self.oauth_state = {}  // simple in-memory store for short-lived "state" parameter. 

		this.server.on('day', ()=>{
			self.oauth_state = {} 
		})  // clear stale oatuh request on daily basis	

		// register oauth login URI (will redirect user to oauth provider login page)
		this.web.addURIHandler('/login/oauth', 'Oauth', function (args, callback) {

			let oauth = self.server.config.get('oauth') || {}
			let profiles = self.config.get('oauth_profiles') || {}
			if (oauth.profile) oauth = Tools.mergeHashes(oauth, profiles[oauth.profile])

			let params = Tools.mergeHashes(args.params, args.query)
			let orig_location = params.orig_location

			if (oauth.enabled && oauth.client_id && oauth.client_secret && oauth.authorize_url) {				
	...
	api_oauth: async function(args, callback) {
		...
		if (!oauth.client_id || !oauth.client_secret || !oauth.enabled || !oauth.user_url || !oauth.token_url || !oauth.redirect_uri) {

			return self.doError('login', "oath is disabled or misconfigured", callback);
		}
		...
		try {
			...
			const tokenResponse = await self.postJsonAsync(oauth.token_url, { 
				data: Tools.mergeHashes(tokenRequestData, oauth.params),
