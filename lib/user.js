...
module.exports = Class.create({
	...
	api_casauth: async function (args, callback) {
		...
		if(session_id) {
			...
			request.request(validateurl, opts, async function (err, resp, data) {
				if (err) {
					...
					try {
						...
						self.logError(3, 'CAS validation failed, response is not json', String(data));
						data = { success: false, message: 'Invalid CAS response' };
						return self.doError('login', "Invalid CAS response", callback);
					}

					const serviceResp = data.serviceResponse;

					if (serviceResp && serviceResp.authenticationSuccess) {
						// CAS ticket is valid, extract user info
						let username = serviceResp.authenticationSuccess.user || serviceResp.authenticationSuccess['user'];
						if (!username) {
			...
			res.writeHead(302, { Location: casUrl + "/cas?service=" + self.server.config.get('base_app_url') + "/api/user/casauth" });
			res.end()	
		}
	},
		...
		self.fireHook('before_login', args, function (err) {
			...
			self.storage.put('sessions/' + session_id, session, function (err, data) {
				if (err) {
					...
					res.writeHead(302, { Location: self.getBaseLocation('index-dev.html#' + orig_location) })
					res.end()			
