...
module.exports = Class.create({
	...
	api_login: async function (args, callback) {
		...
		const createUserAsync = function(path, user) {
			return new Promise((resolve, reject) => {
				self.storage.put(path, user, function (err, data) {
					if (err) {
		...
		const updateUserAsync = function (path, value) {
			return new Promise((resolve, reject) => {
		...
		if (allowGroupAuth) {
			if (!user || useGroupAuth) {
				...
				if (user) {
					...
					if (old_user) {
						await updateUserAsync('users/' + this.normalizeUsername(params.username), user)
						user.created = old_user.created || user.created;
					}
					else {
						await createUserAsync('users/' + this.normalizeUsername(params.username), user)
					}
				}
			}
		}	
