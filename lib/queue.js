...
module.exports = Class.create({
	...
	monitorInternalQueue: function() {
		...
		for (var key in this.internalQueue) {
			...
			if (!task.when || (now >= task.when)) {
				...
				this.logDebug(5, "Processing internal queue task", {id: task.id, when: task.when, title: task.event_title || task.title } );
	...
	shutdownQueue: function() {
		...
		if (this.internalQueue) {
			for (var key in this.internalQueue) {
				var task = this.internalQueue[key];				
				if(parseInt(task.repeat)) { 
					// Do not queue repeat jobs. Those could be in internal queue or running, so you never know if it gets into external queue
					// It's better to launch them on scheduler startup
					this.logDebug(5, "Skipping enqueuing repeat job", task.id);
				}
