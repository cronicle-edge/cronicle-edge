...
module.exports = Class.create({
	...
	loadSession: function (args, callback) {
		...
		// git hub integration
		if (args.request.headers['x-hub-signature'] && self.server.config.get('git_hub_key')) {

			let gitInfo = args.request.params;

			let expectedSignature = "sha1=" + crypto.createHmac("sha1", self.server.config.get('git_hub_key'))
				.update(JSON.stringify(gitInfo))
				.digest("hex");
			if (expectedSignature == args.request.headers['X-Hub-Signature']) {
				return callback(
					null
					, { type: "github", username: (gitInfo.sender || {}).login || 'unknown' }
					, { username: "internal", active: true, privileges: { run_events: 1 } }
				);

			}
			else {
				return callback(new Error("Invalid signature, expected " + expectedSignature), null, null);
			}
		}
		...
		if (session_id) {
			...
			// digest auth	
			// if(session_id == Tools.digestHex(Math.floor(Tools.timeNow(true)/3600) +  self.server.config.get('secret_key'))) { // digest of current hour
			// 	return callback(null, {type: "api", username: "internal"}, { username: "internal", active:true, privileges: {run_events:1} });
			// }
		...
		if (this.temp_keys[api_key]) { // check if temp api key exists
			return callback(null, { type: "api", username: "internal" }, { username: "internal", active: true, privileges: { run_events: 1, abort_events: 1 } });
		}

		// if no api key also check for git hub integration
		if (args.request.headers['x-hub-signature'] && self.server.config.get('git_hub_key') && !api_key) {
		...
		// finally return error if no session or api key detected
