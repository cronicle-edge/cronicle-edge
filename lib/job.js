...
module.exports = Class.create({
		...
		async.series([
			...
			function (err) {
				...
				for (var idx = 0, len = servers.length; idx < len; idx++) {
					...
					if (plugin.secret) job.params['PLUGIN_SECRET'] = plugin.secret;
					if (plugin.id == 'workflow') {  // todo - change to flag
						let temp_key = Tools.digestHex(job.id + (new Date).toDateString())
						self.temp_keys[temp_key] = true
						job.params['TEMP_KEY'] = temp_key
						job.params['BASE_URL'] = 'http://' + self.server.hostname + ':' + self.server.config.get('WebServer').http_port
					}
					...
					// inject xparams as env var (e.g. JOB_PARAM1)
					// if (job.params.sub_params) {
					// 	for (let id in xparams) {
					// 		if (!job[id]) job[id] = xparams[id]
					// 	}
					// }
	...
	abortJob: function (stub) {
		...
		// remove temp key if exist
		// if(job.params['TEMP_KEY']) delete this.temp_keys[job.params['TEMP_KEY']];
		// no need to remove temp key from abort api - it will be cleared by finishJob or server crash
