...
module.exports = Class.create({
	...
	launchLocalJob: function (job) {
		...
		if(job.plugin_secret) {
			try {
				job_env = Tools.mergeHashes(job_env, dotenv.parse(self.decryptObject(job.plugin_secret)))
			}
			catch (e) {	self.logError(6, 'Failed to decrypt plugin secret:', e) }
			delete job.plugin_secret
		}
