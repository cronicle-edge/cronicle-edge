...
module.exports = Class.create({
		...
		async.series([
			...
			function (err) {
				...
				for (var idx = 0, len = servers.length; idx < len; idx++) {
					...
					delete job.timing;
					delete job.ticks;
					if(!job.repeat) delete job.enabled;
	...
	finishLocalJob: function (job) {
        ...
        // if this is Repeat kind of job - put it into queue and return
		if (parseInt(job.repeat) && !job.abort_reason && !this.server.shut && job.enabled) {	
			...
			if (Array.isArray(job.trend)) { // sanity, should always be array
				...
				if (job.trend.length > 16) job.trend.shift()
			...
			fs.stat(job.log_file, (err, stats) =>{
				let maxSize = parseInt(self.server.config.get('log_max_size')) || 16384*2
				if(err) { 
				...
				// If file exceed max value truncate log file
					...
					let repeatInfo = [
						'', '#', '# Repeat job cycle completed'
						, `# cycle seq: ${job.cycles} | job started: ${new Date(parseInt(job.time_start_initial)*1000)} | log size: ${job.log_file_size}`
						, '#', ''
					].join(os.EOL)
			...
			job.when = Tools.timeNow() + parseInt(job.repeat);
