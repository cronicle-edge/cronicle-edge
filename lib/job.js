...
module.exports = Class.create({
	...
	launchLocalJob: function (job) {
		...
		let job_env = Tools.mergeHashes(process.env, job.env || {})

		if(job.secret) {
			try {
                job_env = Tools.mergeHashes(job_env, self.decryptObject(job.secret));
			}
			catch (e) {
		...
		if (job.detached && isUnix) { // only run detached on unix
	...
	monitorServerResources: function (callback) {
		if (this.server.config.get('use_cmd_monitor')) {
			this.monitorServerResourcesPs(callback);
		}
		else {
			this.monitorServerResourcesSi(callback);
		}
	},
	...
	monitorServerResourcesPs: function (callback) {
