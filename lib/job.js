			...
			function (err) {
				...
				for (var idx = 0, len = servers.length; idx < len; idx++) {
					...
					if (plugin.secret) {
						...
						try { 
							job.env['PLUGIN_SECRET'] = plugin.encrypted ? self.decryptObject(plugin.secret) : plugin.secret
						}
						catch {  
							job.env['PLUGIN_SECRET'] = plugin.secret
							// self.logDebug(6, "Failed to  decrypt PLUGIN_SECRET: ", plugin.secret)
						}
