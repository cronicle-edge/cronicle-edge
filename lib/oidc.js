...
function buildOidcSessionMetadata(oauth, idTokenClaims, userInfo, idToken, secret) {
	// Build session fields used for targeted revocation and RP-initiated logout.
	userInfo = userInfo || {};
...
module.exports = Class.create({
	...
	filterOidcRequest: function(args, callback) {
		...
		if (action === 'oidc_logout_redirect') {
			...
			let payload;
			try {
				payload = this.consumeOidcLogoutTicket(secrets.ticket, this.server.config.get('secret_key'));
