...
// copy this one -- the bootstrap below shifts ops off the records, and the engine
// reads the same file to seed records a data folder may be missing
...
module.exports = {
	...
	tests: [
		...
		function testCreateMissingRecords(test) {
			// gomanager above already ran the check, so release the once-per-process latch
			cronicle.missingRecordsChecked = false;

			// simulate a data folder from an older version, which never had global/secrets
			storage.listDelete( 'global/secrets', true, function(err) {
				test.ok( !err, "No error deleting global/secrets" );

				var num_done = 0;
				var onDone = function() {
					if (++num_done < 2) return;

					storage.listGet( 'global/secrets', 0, 0, function(err, items) {
						test.ok( !err, "No error fetching recreated global/secrets" );
						test.ok( items.length == 1, "Recreated global/secrets has one item" );
						test.ok( items[0].id == 'globalenv', "Recreated global/secrets holds the globalenv item" );
						test.ok( items[0].created > 0, "Recreated globalenv item has a creation date" );

						// a record that is already there must be left alone
						cronicle.missingRecordsChecked = false;
						cronicle.createMissingRecords( function() {
							storage.listGet( 'global/secrets', 0, 0, function(err, items) {
								test.ok( !err, "No error fetching global/secrets after second pass" );
								test.ok( items.length == 1, "Existing global/secrets was not seeded again" );
								test.done();
							} );
						} );
					} );
				...
				// two overlapping passes (an extra gomanager) must not seed the record twice
				cronicle.createMissingRecords( onDone );
				cronicle.createMissingRecords( onDone );
			} );
		},

		function testCreateMissingRecordsRollbackSpares(test) {
			// a legitimate write can land between the failed seed and its rollback -- the rollback
			// must leave that record alone, it is not ours to undo
			cronicle.missingRecordsChecked = false;

			storage.listDelete( 'global/secrets', true, function(err) {
				test.ok( !err, "No error deleting global/secrets" );

				// fail the item push, so the seed rolls the key back to nothing
				var orig_list_push = storage.listPush;
				storage.listPush = function(key, item, callback) {
					if (key == 'global/secrets') return callback( new Error("Simulated storage failure") );
					return orig_list_push.apply( storage, arguments );
				};

				// listDelete drops the list lock before its callback, so somebody can create the
				// record for real in the window between the undo and the rollback's read back
				var orig_list_delete = storage.listDelete;
				var injected = false;
				storage.listDelete = function(key, entire, callback) {
					if (!injected && (key == 'global/secrets')) {
						injected = true;
						return orig_list_delete.call( storage, key, entire, function(err) {
							storage.listCreate( 'global/secrets', {}, function() {
								storage.listUnshift( 'global/secrets', { id: 'user_made' }, function() {
									callback(err);
								} );
							} );
						} );
					}
					return orig_list_delete.apply( storage, arguments );
				};

				cronicle.createMissingRecords( function() {
					storage.listPush = orig_list_push;
					storage.listDelete = orig_list_delete;
					test.ok( injected, "The write was injected between the undo and the read back" );

					storage.listGet( 'global/secrets', 0, 0, function(err, items) {
						test.ok( !err, "The record written by somebody else still exists" );
						test.ok( items.length == 1, "The record written by somebody else still has its item" );
						test.ok( items[0].id == 'user_made', "The rollback did not touch a record it did not create" );
						test.done();
					} );
				} );
			} );
		...
		function testCreateMissingRecordsRollbackLock(test) {
			// the rollback's check and delete must be serialized against list writers
			cronicle.missingRecordsChecked = false;
			...
			storage.listDelete( 'global/secrets', true, function(err) {
				...
				// arm only after the undo, whose own listDelete takes the same lock first
				var orig_list_delete = storage.listDelete;
				var armed = false;
				storage.listDelete = function(key, entire, callback) {
					if (key == 'global/secrets') return orig_list_delete.call( storage, key, entire, function(err) {
						armed = true;
						callback(err);
					} );
					return orig_list_delete.apply( storage, arguments );
				...
				var orig_lock = storage.lock;
				var injected = false, write_done = false, write_blocked = null;
				storage.lock = function(key, wait, callback) {
					if (armed && !injected && (key == '|global/secrets')) {
						injected = true;
						return orig_lock.call( storage, key, wait, function(lock_err, lock) {
							// the rollback holds the lock now, so this write has to wait for it
							storage.listUnshift( 'global/secrets', { id: 'queued_write' }, function() { write_done = true; } );
							setTimeout( function() {
								write_blocked = !write_done;
								callback( lock_err, lock );
							}, 100 );
						} );
					}
					return orig_lock.apply( storage, arguments );
				};
				...
				cronicle.createMissingRecords( function() {
					...
					storage.listDelete = orig_list_delete;
					test.ok( injected, "The rollback took the list lock" );
					test.ok( write_blocked === true, "A list write was held off while the rollback held the lock" );

					// and it lands once the rollback lets go
					setTimeout( function() {
						test.ok( write_done, "The held off write completed after the rollback released the lock" );
						test.done();
					}, 250 );
				} );
			} );
		...
		function testCreateMissingRecordsRollbackHeader(test) {
			// listCreate writes the list header before its first page, so a failed page write
			// leaves the header behind -- and listDelete will not take a header off once a page
			// is missing, so the rollback has to notice and remove it directly
			cronicle.missingRecordsChecked = false;

			storage.listDelete( 'global/secrets', true, function(err) {
				test.ok( !err, "No error deleting global/secrets" );

				// fail the first page write, once
				var orig_put = storage.put;
				var page_write_failed = false;
				storage.put = function(key, value, callback) {
					if (!page_write_failed && (key == 'global/secrets/0')) {
						page_write_failed = true;
						return callback( new Error("Simulated storage failure") );
					}
					return orig_put.apply( storage, arguments );
				};

				cronicle.createMissingRecords( function() {
					storage.put = orig_put;
					test.ok( page_write_failed, "The simulated page write failure was reached" );

					storage.get( 'global/secrets', function(err) {
						test.ok( !!err && (err.code == 'NoSuchKey'), "Leftover global/secrets header was removed" );

						// and the next manager start seeds it in full
						cronicle.missingRecordsChecked = false;
						cronicle.createMissingRecords( function() {
							storage.listGet( 'global/secrets', 0, 0, function(err, items) {
								test.ok( !err, "No error fetching global/secrets after header rollback" );
								test.ok( items.length == 1, "global/secrets was seeded in full after header rollback" );
								test.ok( items[0].id == 'globalenv', "Reseeded global/secrets holds the globalenv item" );
								test.done();
							} );
						} );
					} );
		...
		function testCreateMissingRecordsRollback(test) {
			// a record that is only half written must not be left behind, or every later start
			// would see its header and skip the key forever
			cronicle.missingRecordsChecked = false;

			storage.listDelete( 'global/secrets', true, function(err) {
				test.ok( !err, "No error deleting global/secrets" );

				// fail the item push, so the list header is written but the item never is
				var orig_list_push = storage.listPush;
				storage.listPush = function(key, item, callback) {
					if (key == 'global/secrets') return callback( new Error("Simulated storage failure") );
					return orig_list_push.apply( storage, arguments );
				...
				cronicle.createMissingRecords( function() {
					...
					storage.get( 'global/secrets', function(err) {
						test.ok( !!err && (err.code == 'NoSuchKey'), "Half written global/secrets was rolled back" );

						// and the next manager start seeds it in full
						cronicle.missingRecordsChecked = false;
						cronicle.createMissingRecords( function() {
							storage.listGet( 'global/secrets', 0, 0, function(err, items) {
								test.ok( !err, "No error fetching global/secrets after retry" );
								test.ok( items.length == 1, "global/secrets was seeded in full on the retry" );
								test.ok( items[0].id == 'globalenv', "Retried global/secrets holds the globalenv item" );
								test.done();
							} );
						} );
					} );
