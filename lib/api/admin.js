...
module.exports = Class.create({
	...
	redactConfigSecrets: function(confCopy) {
		...
		const redact = function(value) {
			...
			Object.keys(value).forEach(function(key) {
				...
				if (sensitiveKeys.has(normalized) || normalized.endsWith('_password') ||
					normalized.endsWith('_secret') || normalized.endsWith('_token')) {
					value[key] = '[REDACTED]';
				}
	...
	api_get_config: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			// Secrets must never be exposed by Config Viewer, including in debug mode.
			self.redactConfigSecrets(confCopy);
