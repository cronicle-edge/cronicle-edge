...
module.exports = Class.create({
	...
	api_get_config: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			delete confCopy.secret_key // do not transmit secret key
			//confCopy['temp_keys'] = self.temp_keys; // just for monitoring
	...
	api_export: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			var txt = "# Cronicle Data Export v1.0\n" +
				"# Hostname: " + "local" + "\n" +
		...
		this.loadSession(args, function (err, session, user) {
			...
			var queue = async.queue(function (line, callback) {
				...
				if (line.match(/^(\w[\w\-\.\/]*)\s+\-\s+(\{.+\})\s*$/)) {
					...
					// check if key is allowed (should be like global/[...][/0] or users/username)
					// ignore servers, server groups and plugins to avoid interference
					var validKey = key.match(/^global\/(users|schedule|categories|api_keys|conf_keys|secrets)(\/0){0,1}$/g) ||  key.match(/^users\/([\w\.\@]+)$/g)
					if (!validKey) {
					...
					if(key == 'global/schedule/0') info["events"] = cnt
					if(key == 'global/categories/0') info["cats"] = cnt
					if(key == 'global/users/0') info["users"] = cnt
					if(key == 'global/api_keys/0') info["api"] = cnt
					if(key == 'global/conf_keys/0') info["conf"] = cnt
					if(key == 'global/secrets/0') info["secrets"] = cnt

					self.storage.put(key, data, function (err) {
						if (err) {
							// warn("Failed to store record: " + key + ": " + err + "\n");
							result.push({key: key, code:1,  desc: "Failed to import"})
							return callback();
						}
						count++;
						itemCount = Array.isArray(data.items) ? data.items.length : '';
