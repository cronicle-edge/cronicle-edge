...
module.exports = Class.create({
	...
	api_get_live_console: async function (args, callback) {
		...
		self.loadSession(args, function (err, session, user) {
			...
			if(self.server.hostname === job.hostname + 'blah') { 
				...
				return self.get_job_log_tail(params, callback)
			}
				...
				self.logDebug(2, "log", "Reading remote log file", reqParams )	
	...
	api_get_live_log_tail: function (args, callback) {
		...
		const self = this;
		self.logDebug(2, "log", "receive log request", params)
