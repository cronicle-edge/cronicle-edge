...
//##manual changes start here
api_toggle_event: function(args, callback2) {
  var self2 = this;
  var params = args.params;

  this.loadSession(args, function(err, session2, user) {
    if (err)
      return self2.doError("session", err.message, callback2);
    if (!self2.requireValidUser(session2, user, callback2))
      return;

    // Check for disable_enable_events permission
    const canDisableEnableEvents = self2.requirePrivilege(user, "disable_enable_events", callback2, true);

    if (!canDisableEnableEvents)
      return self2.doError("event", "User does not have the required account privileges to perform this action", callback2);

    args.user = user;
    args.session = session2;
    self2.storage.listFind("global/schedule", { id: params.id }, function(err2, event) {
      if (err2 || !event) {
        return self2.doError("event", "Failed to locate event: " + params.id, callback2);
      }

      if (params.hasOwnProperty('enabled')) {
        event.enabled = params.enabled;
        self2.storage.listFindUpdate("global/schedule", { id: params.id }, { enabled: params.enabled }, function(err4) {
          if (err4) {
            return self2.doError("event", "Failed to update event: " + err4, callback2);
          }

          var changeType = params.enabled ? "event_enabled" : "event_disabled";
          self2.logDebug(6, "Successfully updated event: " + event.id + " (" + event.title + ")");
          self2.logTransaction(changeType, event.title, self2.getClientInfo(args, { event: params }));
          self2.logActivity(changeType, { event: params }, args);
          callback2({ code: 0 });
          self2.updateClientData("schedule");

          if (!event.enabled && params.abort_jobs) {
            var all_jobs = self2.getAllActiveJobs(true);
            for (var key in all_jobs) {
              var job2 = all_jobs[key];
              if (job2.event == event.id && !job2.detached) {
                var msg = "Event '" + event.title + "' has been disabled.";
                self2.logDebug(4, "Job " + job2.id + " is being aborted: " + msg);
                self2.abortJob({ id: job2.id, reason: msg });
              }
            }
          }

          var dargs = Tools.getDateArgs(new Date());
          if (params.enabled && event.catch_up && !self2.schedulerGraceTimer && !self2.schedulerTicking && dargs.sec != 59) {
            self2.schedulerMinuteTick(null, true);
          }
          if (self2.eventQueue[event.id]) {
            if (event.queue)
              self2.checkEventQueues(event.id);
            else
              self2.deleteEventQueues(event.id);
          }
        });
      } else {
        return self2.doError("event", "No changes allowed except enabling/disabling", callback2);
      }
    });
  });
},
//##manual changes end here
