...
module.exports = Class.create({
	...
	api_update_event: function (args, callback) {
		...
		this.loadSession(args, async function (err, session, user) {
			if (err) return self.doError('session', err.message, callback);
			if (!self.requireValidUser(session, user, callback)) return;
			const canEditOrToggleEvent = self.hasPrivilege(user, "edit_events") || self.hasPrivilege(user, "disable_enable_events");
			if (!canEditOrToggleEvent) {
				return self.doError("event", "User '" + user.username + "' does not have the required account privileges to perform this action", callback);
			}
