...
module.exports = Class.create({
	...
	api_toggle_event: function(args, callback) {
		...
		this.loadSession(args, (err, session, user) => {
			...
			const canDisableEnableEvents = self.requirePrivilege(user, "disable_enable_events", callback, true);

			if (!canDisableEnableEvents) {
				return self.doError("event", "User does not have the required account privileges to perform this action", callback);
			}

			args.user = user;
			args.session = session;

			self.storage.listFind("global/schedule", { id: params.id }, (err2, event) => {
				if (err2 || !event) {
					return self.doError("event", "Failed to locate event: " + params.id, callback);
				}
				...
				if (params.hasOwnProperty('enabled')) {
					...
					self.storage.listFindUpdate("global/schedule", { id: params.id }, { enabled: params.enabled }, (err4) => {
						if (err4) {
							return self.doError("event", "Failed to update event: " + err4, callback);
						}
						...
						if (!event.enabled && params.abort_jobs) {
							...
							for (let key in all_jobs) {
								...
								if (job2.event == event.id && !job2.detached) {
									...
									self.logDebug(4, "Job " + job2.id + " is being aborted: " + msg);
									self.abortJob({ id: job2.id, reason: msg });
								}
							}
						}
