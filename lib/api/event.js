...
module.exports = Class.create({
	...
	api_get_errors: function (args, callback) {
		// get list of events completed with error. Can pass id parameter to look up specific event
		// this API emulates pagination by scanning entire dataset returned by findEach,
		// but response will only contain job data for jobs between offset and limit.
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.listFindEach('logs/completed', filter, function (item, idx, cb) {
				...
				return callback({ code: 0, rows: jobs, list: { length: total }})
