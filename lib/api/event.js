...
module.exports = Class.create({
	...
	api_toggle_event: function(args, callback2) {
		...
		this.loadSession(args, (err, session, user) => {
			if (err) return self2.doError("session", err.message, callback2);
			if (!self2.requireValidUser(session, user, callback2)) return;
			...
			self2.storage.listFind("global/schedule", { id: params.id }, (err2, event) => {
				if (err2 || !event) {
				...
				if (params.hasOwnProperty('enabled')) {
					...
					self2.storage.listFindUpdate("global/schedule", { id: params.id }, { enabled: params.enabled }, (err4) => {
						if (err4) {
						...
						if (!event.enabled && params.abort_jobs) {
							const all_jobs = self2.getAllActiveJobs(true);
							for (let key in all_jobs) {
								const job2 = all_jobs[key];
						...
						if (self2.eventQueue[event.id]) {
							if (event.queue) self2.checkEventQueues(event.id);
							else self2.deleteEventQueues(event.id);
						}
					});
				} else {
