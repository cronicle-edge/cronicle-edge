	...
	api_config: function(args, callback) {
		...
		var resp = {
			...
			config: Tools.mergeHashes( this.server.config.get('client'), {
				...
				external_users: (this.usermgr.config.get('external_user_api') || this.server.config.get('github_client_id')) ? 1 : 0,
				github_host: this.server.config.get('github_host'),
				github_client_id: this.server.config.get('github_client_id'),
