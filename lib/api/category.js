...
module.exports = Class.create({
	...
	api_create_category: function(args, callback) {
	   ...
	   // make sure title doesn't contain HTML metacharacters
		...
		this.loadSession(args, async function(err, session, user) {
			...
			// first do check on id/title
			let alreadyExist = await self.validateUnique('global/categories', cat, ["id", "title"])
			if (alreadyExist > 0) {
				return self.doError('event', `Category with title or id ${cat.title}/${cat.id} already exists`, callback);
			}
	...
	api_update_category: function(args, callback) {
		...
		this.loadSession(args, async function(err, session, user) {
			...
			self.storage.listFind( 'global/categories', { id: params.id }, async function(err, cat) {
				if (err || !cat) {
				...
				if (cat.title != params.title) {
					let titleExists = await self.validateUnique('global/categories', params, ["title"])
					if (titleExists > 0) {
						return self.doError('event', `Category with title "${params.title}" already exists`, callback);
					}
				}
