...
module.exports = Class.create({
	...
	api_create_plugin: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if(plugin.secret) {
				plugin.secret = self.encryptObject(plugin.secret)
				plugin.encrypted = true // just flag that secret is encrypted
			}
	...
	api_update_plugin: function(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFind( 'global/plugins', { id: params.id }, function(err, plugin) {
				...
				if(params.secret) {
					params.secret = self.encryptObject(params.secret)
					params.encrypted = true // secret wasn't encrypted before 1.7.9
				}
				else {
					params.secret = plugin.secret
				}
