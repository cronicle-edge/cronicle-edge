	...
	api_create_plugin: function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (plugin.secret_value && typeof plugin.secret_value === 'string' ) {
				plugin.secret_preview = Object.keys(dotenv.parse(plugin.secret_value)).join('|')
				plugin.secret = self.encryptObject(plugin.secret_value)
				plugin.encrypted = true
			}
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.listFind('global/plugins', { id: params.id }, function (err, plugin) {
				...
				if(params.hasOwnProperty('secret_value')) { // if user edited secret in textbox
					if(params.secret_value && typeof params.secret_value === 'string') { 
