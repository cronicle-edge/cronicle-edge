...
module.exports = Class.create({
  ...
  defaultWebHookTextTemplates: {
    ...
    job_complete:
      "Job completed successfully on [hostname]: [event_title] [job_details_url]",
    job_failure:
      "Job failed on [hostname]: [event_title]: Error [code]: [description] [job_details_url]",
    job_launch_failure:
      "Failed to launch scheduled event: [event_title]: [description] [edit_event_url]",
  },
  ...
  startup: function (callback) {
    ...
    try {
      mkdirp.sync(this.server.config.get("log_dir") + "/jobs");
    } catch (e) {
      throw new Error(
        "FATAL ERROR: Log directory could not be created: " +
          this.server.config.get("log_dir") +
          "/jobs: " +
          e
      );
    }
    try {
      mkdirp.sync(this.server.config.get("queue_dir"));
    } catch (e) {
      throw new Error(
        "FATAL ERROR: Queue directory could not be created: " +
          this.server.config.get("queue_dir") +
          ": " +
          e
      );
    }
    ...
    if (
      this.server.config.get("ssl_cert_bypass") ||
      this.server.config.get("web_hook_ssl_cert_bypass")
    ) {
    ...
    this.web.addURIHandler(
      /^\/conf$/i,
      "Reports",
      __dirname + "/../htdocs/custom/config.html"
    );
    this.web.addURIHandler(
      /^\/db$/i,
      "Reports",
      __dirname + "/../htdocs/custom/dashboard.html"
    );
    this.web.addURIHandler(
      /^\/console$/i,
      "Console",
      __dirname + "/../htdocs/custom/console.html"
    );
    ...
    if (self.server.config.get("Storage").Filesystem) {
      ...
      this.git = simpleGit(
        self.server.config.get("Storage").Filesystem.base_dir
      );
      this.gitlock = 0;
    }

    // register a handler for HTTP OPTIONS (for CORS AJAX preflight)
    this.web.addMethodHandler(
      "OPTIONS",
      "CORS Preflight",
    ...
    require("uncatch").on(
      "uncaughtException",
      this.emergencyShutdown.bind(this)
    );
    ...
    this.usermgr.registerHook(
      "after_create",
      this.afterUserChange.bind(this, "user_create")
    );
    this.usermgr.registerHook(
      "after_update",
      this.afterUserChange.bind(this, "user_update")
    );
    this.usermgr.registerHook(
      "after_delete",
      this.afterUserChange.bind(this, "user_delete")
    );
    ...
    this.usermgr.registerHook(
      ...
      this.beforeUserLogin.bind(this)
    );
    ...
    this.server.on("minute", function () {
      ...
      if (global.gc) {
        self.gcTimer = setTimeout(function () {
          ...
          self.logDebug(
            10,
            "Forcing garbage collection now",
            process.memoryUsage()
          );
          global.gc();
          self.logDebug(
    ...
    this.checkmanagerEligibility(function () {
      ...
      if (self.server.config.get("manager") && self.multi.eligible)
        self.gomanager();
  ...
  checkmanagerEligibility: function (callback) {
    ...
    this.storage.listGet("global/servers", 0, 0, function (err, servers) {
      ...
      if (
        !Tools.findObject(servers, { hostname: self.server.hostname }) &&
        !self.multi.cluster
      ) {
        ...
        if (!self.multi.worker) {
          self.logDebug(
            4,
            "Server not found in cluster -- waiting for a manager server to contact us"
          );
        }
      ...
      self.storage.listGet(
        ...
        function (err, groups) {
          ...
          for (var idx = 0, len = groups.length; idx < len; idx++) {
            ...
            var regexp = null;
            try {
              regexp = new RegExp(group.regexp);
            } catch (e) {
              self.logError(
                "manager",
                "Invalid group regular expression: " + group.regexp + ": " + e
              );
              regexp = null;
            }
          ...
          if (eligible) {
            self.logDebug(
              4,
              "Server is eligible to become manager (" + group_title + ")"
            );
            self.multi.eligible = true;
          } else {
            self.logDebug(
              4,
              "Server is not eligible for manager -- it will be a worker only"
            );
            self.multi.eligible = false;
          }
  ...
  tick: function () {
    ...
    if (this.numSocketClients) {
      ...
      if (this.multi.manager) {
        ...
        let activeJobs = JSON.parse(
          JSON.stringify(this.getAllActiveJobs(true))
        );
    ...
    var msr_freq = Tools.numKeys(this.activeJobs)
      ? this.server.config.get("monitor_res_freq") || 10
      : 60;
  ...
  managerSocketEmit: function () {
    ...
    if (!count) {
      ...
      this.logDebug(
        8,
        "No manager server socket connection available, will retry"
      );
  ...
  updateClientData: function (name) {
    ...
    this.storage.listGet("global/" + name, 0, 0, function (err, items) {
      if (err) {
        self.logError(
          "storage",
          "Failed to fetch list: global/" + name + ": " + err
        );
        return;
      }
      ...
      if (name == "plugins") {
        // clear out secrets
        if (Array.isArray(itemCopy))
          itemCopy.forEach((plug) => delete plug.secret);
      }
  ...
  beforeUserLogin: function (args, callback) {
    ...
    async.each(
      ["schedule", "categories", "plugins", "server_groups"],
  ...
  gitSync: async function (message, callback) {
    ...
    if (self.gitlock)
      return callback(
        new Error("Git Sync Failed: sync is locked by another process"),
        null
      );
    self.gitlock = 1;
    ...
    if (!self.git || !gitConf.enabled || !isRepo)
      return callback(new Error("Git Sync: git is not set or enabled"), null);

    let gitAdd = (gitConf.add || "global,users")
      .split(/[,;|]/)
      .map((e) => e.trim())
  ...
  fireInfoHook: function (web_hook, data, logMessage) {
    ...
    if (typeof web_hook === "string") {
      ...
      return self.logDebug(
        9,
        "Web Hook Error: Invaid data type (string or object expected"
      );
    }
    // combine global and hook specific options (if specified)
    let wh_options = Tools.mergeHashes(
      self.server.config.get("web_hook_custom_opts") || {},
      wh_config.options || {}
    );
    ...
    if (wh_config.token)
      wh_headers["Authorization"] = "Bearer " + wh_config.token;
    ...
    self.request.json(
      ...
      function (err, resp, data) {
        // log response
        if (err)
          self.logDebug(9, "Web Hook Error: " + wh_config.url + ": " + err);
        else
          self.logDebug(
            9,
            "Web Hook Response: " +
  ...
  logActivity: function (action, orig_data, args) {
    ...
    try {
      switch (action) {
        ...
        // api keys
        case "apikey_create":
          msg = `🔑 New API Key created: *${data.api_key.title}* ${
            ("" + data.api_key.key).substr(0, 4) + "******"
          } `;
          actionType = "update";
          item = data.api_key.title;
          break;
        case "apikey_update":
          msg = `🔑 API Key updated: *${data.api_key.title}*  ${
            ("" + data.api_key.key).substr(0, 4) + "******"
          } `;
          actionType = "update";
          item = data.api_key.title;
          break;
        case "apikey_delete":
          msg = `🔑 API Key deleted: *${data.api_key.title}*  ${
            ("" + data.api_key.key).substr(0, 4) + "******"
          } `;
          actionType = "update";
          item = data.api_key.title;
        ...
        // conf keys
        case "confkey_create":
          msg = `🔧 Config Key created: *${data.conf_key.title}* : ${data.conf_key.key} `;
          actionType = "update";
          item = data.conf_key.title;
          break;
        case "confkey_update":
          msg = `🔧 Config Key updated: *${data.conf_key.title}* : ${data.conf_key.key} `;
          actionType = "update";
          item = data.conf_key.title;
          break;
        case "confkey_delete":
          msg = `🔧 Config Key deleted: *${data.conf_key.title}* : ${data.conf_key.key} `;
          actionType = "update";
          item = data.conf_key.title;
          break;
        ...
        case "job_run":
          msg =
            "📊 Job *#" + data.id + "* (" + data.title + ") manually started ";
        ...
        case "job_complete":
          if (!data.code) {
            ...
            msg =
              ...
              data.hostname.replace(/\.[\w\-]+\.\w+$/, "") +
              "* failed with error: " +
              encode_entities(data.description || "Unknown Error").replace(
                /\x1B\[[0-?]*[ -/]*[@-~]/g,
                ""
              );
          }
          msg += ` <${baseUrl}/#JobDetails?id=${
            data.id
          } | Job Details> _- ${new Date().toLocaleTimeString()}_ `;
          actionType = "job";
          item = data.id;
          break;
        case "job_delete":
          msg =
            "📊 Job *#" + data.id + "* (" + data.title + ") manually deleted";
          actionType = "update";
          item = data.id;
          break;
        ...
        case "job_failure": // xxx
          ...
          msg += `<${this.server.config.get("base_app_url")}/#JobDetails?id=${
            data.job.id
          } | More details> _- ${new Date().toLocaleTimeString()}_ `;
          actionType = "job";
          item = data.job.id;
          break;
        ...
        case "state_update":
          msg =
            "ℹ️ Scheduler manager switch was *" +
    ...
    if (
      ...
      this.gitSync(gitMsg, (err, data) => {
        if (err) {
          return self.logDebug(9, "Git Sync Error: " + err);
        }
        self.logDebug(9, "Git Sync Success: ");
      });
    }

    // manual git sync (only if click on backup button)
    if (gitConf.enabled && action == "backup") {
      this.gitSync("manual backup", (err, data) => {
        if (err) {
          if (adminHook) {
            // for debugging purposes send git error to admin webhook
            self.request.json(
              adminHook,
              { text: "Git Sync Failed: " + err.message },
              (err, resp, data) => {
  ...
  managerFailover: function () {
    ...
    this.logDebug(
      3,
      "No manager ping received within " +
        this.server.config.get("manager_ping_timeout") +
        " seconds, choosing new manager"
    );
    ...
    async.series(
      ...
      function (err) {
        ...
        if (!candidates[self.server.hostname]) {
          self.logDebug(
            4,
            "We are no longer eligible for manager, going into idle mode"
          );
          self.multi.managerHostname = "";
          self.multi.manager = self.multi.worker = self.multi.cluster = false;
          return;
        ...
        self.logDebug(
          6,
          "We are rank #" + rank + " for manager, pinging superiors",
          superiors
        );
        ...
        async.each(
          ...
          function (hostname, callback) {
            ...
            var api_url =
              self.getServerBaseAPIURL(hostname, server.ip) + "/app/status";
            self.logDebug(
              10,
              "Sending API request to remote server: " +
                hostname +
                ": " +
                api_url
            );
            ...
            self.request.json(
              ...
              function (err, resp, data) {
                if (err) {
                  self.logDebug(
                    10,
                    "Failed to contact server: " + hostname + ": " + err
                  );
                  return callback();
                }
                ...
                if (data.tick_age > 60) {
                  self.logDebug(
                    1,
                    "WARNING: Failed to ping server: " +
                      hostname +
                      ": Tick age is " +
                      Math.floor(data.tick_age) +
                      "s",
                    data
                  );
                  return callback();
          ...
          function () {
            if (alive.length) {
              self.logDebug(
                6,
                alive.length +
                  " servers ranked above us are alive, so we will become a worker",
                alive
              );
            } else {
              self.logDebug(
                5,
                "No other servers ranked above us are alive, so we are the top candidate for manager"
              );
              self.gomanager();
            }
          } // pings complete
  ...
  managerConflict: function (worker) {
    ...
    var err_msg =
      "manager CONFLICT: The server '" +
      worker.hostname +
      "' is also a manager server. Shutting down immediately.";
          ...
          if (file.match(/\/(\w+)(\-detached)?\.json$/)) {
            ...
            fs.readFile(file, { encoding: "utf8" }, function (err, data) {
              ...
              if (job) {
                self.logDebug(
                  5,
                  "Recovering job data: " + job_id + ": " + file,
                  job
                );
                ...
                if (job.detached && job.pid && self.isProcessRunning(job.pid)) {
                  ...
                  self.logDebug(
                    ...
                    "Detached PID " +
                      job.pid +
                      " is still alive, resuming job as if nothing happened"
                  );
                  ...
                  job.detached &&
                  fs.existsSync(
                    self.server.config.get("queue_dir") +
                      "/" +
                      job_id +
                      "-complete.json"
                  )
                  ...
                  self.logDebug(
                    ...
                    "Detached job " +
                      job_id +
                      " completed on its own, skipping recovery (queue will pick it up)"
                  );
          ...
          glob(log_spec, {}, function (err, files) {
            ...
            async.eachSeries(
              ...
              function (file, callback) {
                ...
                if (file.match(/\/(\w+)(\-detached)?\.log$/)) {
                  ...
                  if (!self.activeJobs[job_id]) {
                    self.logDebug(
                      5,
                      "Recovering job log: " + job_id + ": " + file
                    );
                    self.uploadJobLog(
                      {
                        id: job_id,
              ...
              function (err) {
                ...
                glob(
                  self.server.config.get("log_dir") + "/jobs/*.tmp",
                  {},
                  function (err, files) {
          ...
          self.storage.listGetInfo(list_path, function (err, info) {
            ...
            if (info.length > max_rows) {
              ...
              self.logDebug(
                ...
                "List " +
                  list_path +
                  " has grown too long, trimming to max: " +
                  max_rows,
                info
              );
  ...
  listRoughChop: function (list_path, max_rows, callback) {
    ...
    this.logDebug(
      4,
      "Roughly chopping list: " + list_path + " down to max: " + max_rows
    );
    ...
    self.storage._listLock(list_path, true, function () {
      ...
      self.storage.listGetInfo(list_path, function (err, list) {
        ...
        if (err) {
          self.logError(
            "maint",
            "Failed to load list header: " + list_path + ": " + err
          );
          ...
          function (callback) {
            ...
            self.storage._listLoadPage(
              ...
              function (err, page) {
                ...
                self.storage.delete(
                  list_path + "/" + list.last_page,
                  function (err) {
                    if (err) {
                      // log error but don't fail entire op
                      self.logError(
                        "maint",
                        "Failed to delete list page: " +
                          list_path +
                          "/" +
                          list.last_page +
                          ": " +
          ...
          function (err) {
            ...
            self.storage.put(list_path, list, function (err) {
              if (err) {
                self.logError(
                  "maint",
                  "Failed to update list header: " + list_path + ": " + err
                );
              }
  ...
  deleteExpiredJobData: function (key, data, callback) {
    ...
    this.storage.delete(key, function (err) {
      if (err)
        self.logError(
          "maint",
          "Failed to delete expired job data: " + key + ": " + err
        );
      ...
      self.storage.delete(log_key, function (err) {
        if (err)
          self.logError(
            "maint",
            "Failed to delete expired job log: " + log_key + ": " + err
          );
    ...
    callback(
      "200 OK",
      {
        ...
        "Access-Control-Allow-Headers":
          args.request.headers["access-control-request-headers"] || "*",
  ...
  shutdown: function (callback) {
    ...
    var self = this;
    if (!self.normalShutdown)
      this.logActivity("server_sigterm", { hostname: this.server.hostname });
      ...
      async.whilst(
        ...
        function (callback) {
          if (first) {
            self.logDebug(
              3,
              "Waiting for " + count + " active jobs to complete",
              ids
            );
            first = false;
          }
