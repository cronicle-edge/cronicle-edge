...
module.exports = Class.create({
	...
	isManagerGroup: function (group) {
		...
		// treat either flag as manager-eligible on the read path (stored records are never rewritten)
		return !!(group.manager || group.master);
	},
	...
	createMissingRecords: function (callback) {
		// create global records this version expects but an older data folder may not have,
		// e.g. global/secrets, which is read on every job launch (jobs fail if it is not there).
		// records that already exist are never touched, so this is a no-op on a healthy install.
		const self = this;
		...
		// key as missing and seed it twice, so only the first pass of a process does the work
		if (this.missingRecordsChecked) { if (callback) callback(); return; }
		this.missingRecordsChecked = true;

		var setup = Tools.copyHash(require('../sample_conf/setup.json'), true);
		var minimal = (process.env['CRONICLE_setup'] === 'minimal');

		// group the setup records by key, keeping setup.json order
		var records = [];
		setup.storage.forEach(function (params) {
			// global/users is left out on purpose -- seeding it would list a default admin
			// account on an install that has none
			if ((params[1] == 'global/users') || !params[1].match(/^global\//)) return;
		...
		async.eachSeries(records,
			function (record, callback) {
				self.storage.get(record.key, function (err) {
					// only a missing key may be seeded -- on any other error we cannot tell what
					// is in there, so the record is left alone
					if (!err || (err.code != 'NoSuchKey')) return callback();

					async.eachSeries(record.ops,
						function (params, callback) {
							...
							if (typeof (args[1]) == 'object') {
								var obj = args[1];
								if (minimal && obj.optional) return callback();
								if (obj.created) obj.created = Tools.timeNow(true);
								if (obj.modified) obj.modified = Tools.timeNow(true);
								if (obj.regexp && (obj.regexp == '_HOSTNAME_')) obj.regexp = '^(' + Tools.escapeRegExp(self.server.hostname) + ')$';
								if (obj.hostname && (obj.hostname == '_HOSTNAME_')) obj.hostname = self.server.hostname;
								if (obj.ip && (obj.ip == '_IP_')) obj.ip = self.server.ip;
							}

							args.push(callback);
							self.storage[func].apply(self.storage, args);
						},
						function (err) {
							if (!err) {
								self.logDebug(3, "Created missing record: " + record.key);
								return callback();
							}

							self.logError('storage', "Failed to create missing record: " + record.key + ": " + err);

							// we only get here for a key that did not exist before this pass, so a
							// partial write can be undone -- left in place, its header would make
							// every later start skip the key and the record would stay half seeded
							var is_list = !!record.ops[0][0].match(/^list/);
							var lock_key = '|' + record.key; // same key the list ops lock, see _listLock
							var undo = is_list ?
								function (cb) { self.storage.listDelete(record.key, true, cb); } :
								function (cb) { self.storage.delete(record.key, cb); };

							undo(function () {
								// do not trust the undo: listCreate writes the list header before its
								// first page, and listDelete gives up on the header once a page is
								// missing, so check what is actually left -- under the list lock, or a
								// legitimate write could land between the check and the delete
								self.storage.lock(lock_key, true, function (lock_err) {
									if (lock_err) {
										self.logError('storage', "Record " + record.key + " is left half seeded and will not self heal: " + lock_err);
										return callback();
									}
									var done = function () { self.storage.unlock(lock_key); callback(); };

									self.storage.get(record.key, function (get_err) {
										if (get_err && (get_err.code == 'NoSuchKey')) return done();

										self.storage.get(record.key + '/0', function (page_err) {
											// only our own residue is a header with no first page: a write
											// that completed always has its page 0, and if another writer
											// repopulated the key the seeding goal is met either way, so
											// never remove a record we cannot prove this pass created
											if (!is_list || !page_err || (page_err.code != 'NoSuchKey')) {
												self.logError('storage', "Not removing " + record.key + " after the failed seed: it has a first page, so it is not ours to undo");
												return done();
											}
											// the header is all that has to go -- any surviving page is an
											// orphan file the next pass overwrites when it creates the list
											self.storage.delete(record.key, function (del_err) {
												if (del_err) self.logError('storage', "Record " + record.key + " is left half seeded and will not self heal: " + del_err);
												done();
											});
										});
									});
	...
	gomanager: function () {
		...
		// seed global records a data folder from an older version may be missing -- the manager
		// does this so two servers cannot race to create the same record
