...
module.exports = Class.create({
	...
	safeJobLog: function(job) { // print less verbose, more readable job data on logging
		if(!job) return ''
		const logConfig = this.server.config.get('safe_job_log');
		const formatSubKeys = (keyObject) => {
			logConfig['exclude_stringify_keys']?.forEach(sk => delete keyObject[sk]);
			return JSON.stringify(keyObject);
		};
		const formatValue = (k) => logConfig['stringify_keys']?.includes(k) ? formatSubKeys(job[k]) : job[k];
		return Object.keys(job).filter(e => !logConfig['exclude_keys']?.includes(e)).map(k => k + ': ' + formatValue(k)).join(" | ")
	},
