...
module.exports = Class.create({
	...
	startup: function (callback) {
		...
		this.web.addURIHandler(/^\/conf$/i, 'Reports', __dirname + "/../htdocs/custom/config.html");
	...
	decrypt: async function(message) {
		let encOpts = { inform: 'PEM', inkey: this.server.config.get('cms_key') || process.env['CMS_KEY'] || 'conf/cronicle.key' }
		return openssl('cms.decrypt', Buffer.from(message), encOpts)
	},

	encrypt: async function(cipher) {
		let encOpts = { outform: 'PEM', recip: this.server.config.get('cms_key') || process.env['CMS_KEY'] || 'conf/cronicle.key' }
