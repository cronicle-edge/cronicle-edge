...
module.exports = Class.create({
	...
	decrypt: async function(message) {
		let encOpts = { inform: 'PEM', inkey: this.server.config.get('CMS_KEY') || 'conf/cronicle.key' }
		return openssl('cms.decrypt', Buffer.from(message), encOpts)
	},

	encrypt: async function(cipher) {
		let encOpts = { outform: 'PEM', recip: this.server.config.get('CMS_KEY') || 'conf/cronicle.key' }
