...
stream.on('json', function(job) {
	...
	var classPath = path.join(__dirname, 'jars/*') + ':/jars/*:' + tmpDir;
	if(job.params.cp_param) { classPath = job.params.cp_param + ":" + classPath}
	if(process.env['CLASSPATH']) {classPath = process.env['CLASSPATH'] + ":" + classPath; }
	process.env['CLASSPATH'] = classPath;
	var java_file =  path.join( tmpDir, className + '.java')
	var script_file = path.join(tmpDir, 'run.sh' );
	var java = "java"
	var javac = "javac"
	if(job.params.jh_param) {
		javac = path.join(job.params.jh_param, 'bin', 'javac')
		java = path.join(job.params.jh_param, 'bin', 'java')
	}
	var script = `#!/bin/sh
		 ${javac} ${java_file}
		 ${java} ${className}
