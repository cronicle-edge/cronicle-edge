...
### Backfill storage from an older version
If you point cronicle-edge at a data folder that was created by an older cronicle version, some global records edge expects may not be there - most notably `global/secrets`, which is read on every job launch (jobs would fail with `Failed to fetch key: global/secrets: File not found`). The `setup` command cannot help here, it stops as soon as it sees an existing setup. Stop cronicle and run `bin/storage-cli.js backfill` instead: it walks the same setup.json record list, creates only the `global/*` keys that are missing, and leaves every record that already exists untouched - so running it twice does nothing the second time. User accounts are not seeded, use the `admin` command if you need a new admin account.
